...
pub fn anyvalue_strategy(
    ...
    (0..num_possible_types).prop_flat_map(move |i| {
        ...
        match selection {
            ...
            _ if selection == S::BINARY => prop::collection::vec(any<u8>(), 0.100).prop_map(|vec| {
                let leaked: &'static [u8] = Box::leak(vec.into_boxed_slice());
                AnyValue::Binary(leaked)
            }).boxed(),
            _ if selection == S::BINARY_OWNED => prop::collection::vec(any::<u8>(), 0..100).prop_map(AnyValue::BinaryOwned).boxed(),
            #[cfg(feature = "object")]
